...
function EditProfileForm({ setProfileImage, profileImage, currentUser, shows, pronouns, sexualities }) {
	...
	function handleProfilePicSubmit(e) {
		...
		fetch('/profile_photos', {
			...
			.then((res) => res.json())
			.then((data) => setProfileImage(data.image_url));
		// history.push('/')
	}
	...
	return (
		<div>
			<form id="edit-profile-div" onSubmit={handleProfileUpdate}>
				<label id="edit-profile-content-div" htmlFor="first_name">
					First Name:
				</label>
				<input
				...
				/>
				<label id="edit-profile-content-div" htmlFor="last_name">
					Last Name:
				</label>
				<input
					...
					onChange={handleChange}
				/>
				<label id="edit-profile-content-div" htmlFor="bio">
					Bio:
				</label>
				<input id="bio-edit-input" type="text" name="bio" value={formData.bio} onChange={handleChange} />
				<div>
